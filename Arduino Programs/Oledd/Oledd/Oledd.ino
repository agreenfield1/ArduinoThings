...
void setup()
{
  ...
  if (Hour>9) {
     dtostrf(Hour,2,0,tempChar);
  } else {
     dtostrf(Hour,1,0,tempChar);
  }
  strcat (Line1,tempChar);
  
  strcat (Line1,":");

  dtostrf(Minute,2,0,tempChar);
  if (tempChar[0]== ' ') {
    tempChar[0]='0';
  }
  ...
  sendChar(Line1);
  //Line1str=String(11)+":"+String(Minute)+":"+String(Second)+" am";
  //Line1str.toCharArray(Line1,20);
  //sendString(Line1str);  
  delay(5000);
 // clearOLED();
  //1 　　line
...
// we switch back and forth between the command tables  and i try to put extra lines beteen the switches
 ...
 sendCommand(0x71);  // function selection A, 
   sendData(0x00);   //  enable internal Vdd regulator (if  you use 4.5-5.5  volts)
...
//
///////////////////////////////////////////////////////////////////////////////////////
//void sendString(String Stringg) {
//  unsigned char i=0;
 // char charString[20]="";
 // Stringg.toCharArray(charString,20);
//  while(charString[i])sendData(charString[i++]);      // *** Send String to OLED
//}

void sendChar(const char *charString) {
  unsigned char i=0;
  while(charString[i])sendData(charString[i++]);      // *** Send String to OLED
}
////////////////////////////////////////////////////////////////////////////////////////
